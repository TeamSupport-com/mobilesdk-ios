...
class SettingsViewController: UIViewController, UITableViewDataSource {
    ...
    @IBOutlet weak var baseJsUrlTextField: UITextField!
    @IBOutlet weak var entryPageTextField: UITextField!
    @IBOutlet weak var baseInstanceUrlTextField: UITextField!
    ...
    @objc func dismissKeyboard() {
        ...
        self.baseJsUrlTextField.resignFirstResponder()
    }

    @IBAction func save(_ sender: Any) {
        guard
                let jsUrlString = baseJsUrlTextField.text,
                let baseJsUrl = URL(string: jsUrlString),
                ...
                let baseInstanceUrlString = baseInstanceUrlTextField.text,
                let baseInstanceUrl = URL(string: baseInstanceUrlString)
                else {
            self.showNotValidUrlsPopup()
            return
        }

        let configuration = ChatConfiguration(widgetId: widgetId, baseJsUrl: baseJsUrl, provider: provider, entryPageUrl: entryPageUrl, baseInstanceUrl: baseInstanceUrl, customVariables: self.createCustomVariablesDictionary())
